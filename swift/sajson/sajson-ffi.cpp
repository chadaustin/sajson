...
static sajson::document* unwrap(sajson_document* doc) { return static_cast<sajson::document*>(doc); }

static sajson_document* wrap(sajson::document* doc) { return static_cast<sajson_document*>(doc); }
...
size_t sajson_get_error_line(sajson_document* doc) { return unwrap(doc)->get_error_line(); }

size_t sajson_get_error_column(sajson_document* doc) { return unwrap(doc)->get_error_column(); }

const char* sajson_get_error_message(sajson_document* doc) { return unwrap(doc)->get_error_message_as_cstring(); }

uint8_t sajson_get_root_type(sajson_document* doc) { return unwrap(doc)->_internal_get_root_type(); }

const size_t* sajson_get_root(sajson_document* doc) { return unwrap(doc)->_internal_get_root(); }

const unsigned char* sajson_get_input(sajson_document* doc) {
    return reinterpret_cast<const unsigned char*>(unwrap(doc)->_internal_get_input().get_data());
}

size_t sajson_get_input_length(struct sajson_document* doc) { return unwrap(doc)->_internal_get_input().length(); }
...
size_t sajson_find_object_key(const size_t* const payload, const char* key, size_t length, const unsigned char* input) {
    ...
    const sajson::internal::object_key_record* start
        = reinterpret_cast<const sajson::internal::object_key_record*>(payload + 1);
    ...
    return (i != end && (i->key_end - i->key_start) == key_string.length()
               && memcmp(key_string.data(), input + i->key_start, key_string.length()) == 0)
        ? i - start
        : value_length;
}
